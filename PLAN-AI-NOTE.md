...
| 8 | ✅ 완료 | (이번) | 머리칸 버튼 묶음을 `<span class="anacts">` 로 감싸고(자리 모자라면 그 안에서 옆으로 스크롤, X 는 늘 보임), `#an-copy/#an-export/#an-regen` 을 켜고 `#an-tasks` 를 새로 추가(레일의 `.antasks` 는 폐지, `.anfunote` 안내문으로 대체 — ≤900px 에서 레일이 숨어도 TASKS 를 못 쓰는 문제 해소). `anPlainText(rec)` → **COPY**(`navigator.clipboard` 실패 시 숨은 textarea + `execCommand` 로 폴백). **EXPORT** = `body.anprinting` 표식 + `@media print`(앱 나머지 숨기고 흰 시트만, 레일·머리칸·그림자 제거, `@page margin:16mm`) + `window.print()`, `afterprint`·800ms 폴백으로 표식 해제. **REGENERATE** = `editedByUser` 면 `confirm()`, 도는 동안 `#an-mt`="다시 만드는 중…" + 버튼 다섯 잠금 → `makeAiNote(id)` 다시 부르고 끝나면 `renderAiNote`. **TASKS** = `note.actions` 를 `KV.tasks[id]` 에 덧붙임(같은 문장·`텍스트 — 담당 / 마감` 둘 다로 중복 제거), `saveKV("tasks")`·`renderRight()`·토스트. `renderAiNote` 가 편집 중·`S.anBusy` 면 네 버튼 `disabled`, `anStartEdit` 도 `S.anBusy` 면 무시. ≤640px 에서 버튼 이름만 줄이고(접기 규칙 폐지) 스크롤로 접근. localhost 로 COPY 문자열·TASKS 덧붙임/중복제거·편집 잠금·REGENERATE 교체·`editedByUser` 소거·인쇄 표식 토글까지 확인. **`file://` 로 실제 인쇄 미리보기·클립보드·IndexedDB 저장만 한 번 봐 주세요.** |
| 9 | 🔶 코드·로컬 점검 완료 / `file://` 사용자 확인 대기 | (이번) | **통합 감사**: 1~8단계 접점 전부 재확인 — `#rbody` 위임의 `data-annote="open"→openAiNote`, `removeDoc`·`#btn-wipe` 의 `dbDel/dbClear("ainote")`+메모리 정리, `exportBackup`/불러오기의 `ainote` 왕복, `loadAll` 의 `dbAll("ainote")`, `DB_VER=4`+`openDB` 스토어 한 줄, `aiNoteBlock` 상태(`· 수정함` 포함), 설정 모달 `#an-*` 7개+`syncAiNoteRow`, 모델 승격 정규식에 `gemini-3.5-flash` 포함 — 모두 정상. **로컬 스모크**(콘솔에서 `callAINote` 스텁): 생성→모달→편집·저장(번호 재부여·`editedByUser`)→TASKS(중복 제거)→REGENERATE(노트 교체·표식 소거)→백업 왕복→Esc 닫기 전부 통과, 콘솔 오류 0. **고친 것**: `anCopy` 를 `await` 없이 — 옛 `execCommand` 를 클릭 순간에 먼저 시도하고(그게 `file://` 에서 됨) 실패 시 `navigator.clipboard` 로. **남은 것 = 사용자가 `file://` 에서**: 실제 키로 한 바퀴, COPY 붙여넣기, EXPORT 인쇄 미리보기(흰 시트만), IndexedDB 저장·새로고침, 백업 zip 왕복. 확인되면 이 표에 ✅ 로 바꾸고 마무리 커밋. |
...
## 7. 8단계 상세 (✅ 완료. 아래는 그때 쓴 지시서, 기록용으로 남김)
...
## 8. 10단계 — 생성 품질 고도화 (프롬프트 재작성)

작성 2026-09-09. 1~9단계로 **틀**은 다 만들어졌고, 이 단계는 **AI가 쓰는 글의 질**만 손봤습니다.
코드 구조·스키마·화면·저장 방식은 하나도 바뀌지 않았습니다.

### 왜 고쳤는가

옛 프롬프트에는 이런 줄이 있었습니다 — `회의록의 표현을 바꾸지 말고 그대로 옮긴다`.
그래서 사용자가 회의 중에 급히 적은 조각(`단가 얘기 다시 ㄱㄱ 원가팀`)이 그 모양 그대로 회의록에 실렸습니다.
받아적은 메모를 **옮겨 적는** 기능이었지, 회의록을 **써 주는** 기능이 아니었습니다.

### 무엇을 바꿨는가 (전부 `11-3) AI NOTE` 구획 안, 약 5316~5400행)

| 바꾼 것 | 내용 |
|---|---|
| 첫 문단 | "원문은 급히 받아적은 메모다 — 옮겨 적지 말고, 끝까지 읽고 맥락과 의도를 파악해 임원이 읽을 정식 회의록으로 **다시 써라**" 로 역할을 새로 정의. |
| `AN_SHAPE` 맨 앞 `"_read"` | **새 메모칸.** AI 는 JSON 을 위에서 아래로 써 내려가므로, 먼저 이 칸에 '무슨 회의였는지' 를 3~5줄 정리하게 하면 그 아래 칸을 훨씬 정확히 채웁니다. 화면에는 안 쓰이고 `aiNoteParse()` 가 읽지 않아 저절로 버려집니다. **지우지 마세요.** |
| `--- 어떻게 다시 쓰는가 ---` (신설) | 조각 → 완결 문장(빠진 주어·목적어를 문맥에서 채움) · 흩어진 같은 주제 한자리로 모으기 · 중복 합치기 · 구어체/줄임말/은어/오타 정돈 · `~함 / ~하기로 함 / ~할 것` 회의록 종결형 · 잡담 버리기 · 한 항목엔 한 가지 이야기. |
| `--- 이렇게 다듬습니다 ---` (신설) | 원문 → 다시 쓴 문장 예시 3개(A 한 건 · 지시 한 건 · Q/A 쌍 분리). 문체의 기준점입니다. |
| `--- 절대 지킬 것 (지어내기 금지) ---` (신설) | **다듬는 것은 '표현' 뿐, '사실' 은 절대 손대지 않는다.** 숫자·금액·비율·날짜·사람/팀/회사/제품 이름은 원문에 있는 것만. 없는 결론·배경·이유를 채우지 않음. 뜻을 끝내 모르겠으면 원문 표현 그대로 둠. — 다시 쓰라고 풀어 준 만큼 이 울타리가 반드시 같이 있어야 합니다. |
| `qa` 지침 | 팀 순서 = 원문에 처음 나온 순서. 뒤쪽에 같은 팀이 또 나오면 **새 팀을 만들지 말고 앞 팀에 이어 붙임**. 팀이 안 적혔으면 문맥 판단, 그래도 모르면 `기타`. 한 줄에 붙은 질문+답은 Q·A 두 항목으로 나눔. |
| `summary` 지침 | 무게가 큰 것부터. `bold` 는 **먼저 쓴 `text` 안에서 글자 그대로 잘라 낼 것**(다시 쓴 문장이므로 원문에서 베끼면 밑줄이 사라짐 — 파서가 버립니다). |
| `actions` 지침 | `qa` 의 '지시' 항목과 앞뒤가 맞을 것. 그 한 줄만 읽어도 무엇을 어떻게 하라는 건지 알게. |
| Agenda 안내문 | "어느 안건 이야기인지 판단할 때 쓰고, **Agenda 에만 있고 회의에서 다루지 않은 내용은 넣지 말 것**" 을 덧붙임. |
| 마지막 규칙 | "원문이 이미 잘 정리된 글이면 굳이 뒤집지 말고 문체만 고르게 다듬는다" — 정성껏 쓴 회의록을 망치지 않기 위한 안전장치. |

### 손대지 않은 것

- `AN_LIM` 수치, JSON 필드 이름·구조, `aiNoteParse()` 의 거르는 로직 (주석 한 줄만 추가)
- `callAI` 의 `temperature: 0` — 사실을 지켜야 하는 작업이라 그대로 둡니다
- 모달·편집·COPY/EXPORT/REGENERATE/TASKS·저장·백업 전부

### 확인한 것 / 남은 것

- localhost 로 페이지 로드 → 콘솔 오류 0, `aiNotePrompt()` 가 3,465자로 정상 생성, `aiNoteParse()` 가 `_read` 를 버리고 `bold` 검증·'지시' 뒤로 정렬·`due` 기본값까지 종전대로 동작 (모의 응답으로 확인).
- **남은 것 = 사용자가 `file://` 에서 실제 키로 한 바퀴.** 평소처럼 두서없이 적은 회의록으로 `Generate AI NOTE` 를 눌러 보고, ① 문장이 보고서 문체로 다시 쓰였는지 ② 숫자·이름·날짜가 원문과 똑같은지(**지어낸 것이 없는지 꼭 확인**) ③ 같은 팀 이야기가 한자리에 모였는지 ④ Executive Summary 밑줄이 살아 있는지 봐 주세요.
