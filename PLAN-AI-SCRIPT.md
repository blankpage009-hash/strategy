...
- PDF 그림 경로도 확인(가짜 fetch 로 진짜 `callAI` 요청 본문을 봄): 뷰어에 열린 `S.pdf`(live) 경로에서 2~4p 범위 → 그림 3장 + 라벨 `[2p]·[3p]·[4p]`,
  `S.pdf` 가 없을 때 IDB 원본 경로, 그림 루프 도중 `S.pdf` 를 destroy 해도 IDB 원본으로 이어서 4장 완성, `res.src === "text+image"`.
  `makeAiSum` 은 `pageNos` 없이 부르므로 라벨이 예전 그대로(`[1쪽]`)임을 확인.
- 주의: pdf.js 의 `page.render` 는 숨겨진 탭(document.hidden)에서는 rAF 가 멈춰 끝나지 않습니다 — 브라우저 창을 보이게 두고 만들어야 합니다(기존 AI 요약도 같음).
- 실제 제미나이 답의 품질(문체·분량·틀 준수)은 file:// 에서 사용자가 직접 확인해야 합니다.
