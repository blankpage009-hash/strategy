...
| 5 | 레일 필터 · 커맨드바 교집합 · 최근 검색어/열람 저장 · 좁은 화면 · 설명서 · 옛 코드 정리 | 하~중 | Sonnet 5 · 중간 | **[완료]** 2026-09-18 · 커밋 `58fa9c9` |
...
### [5단계] 마무리 — 난이도 하~중 · Sonnet 5 · 노력 중간 — **[완료] 2026-09-18 · 커밋 `58fa9c9`**

**실제로 한 것** (아래 원래 계획 1~6, 5번은 **2단계에서 이미 끝나 있었음** 확인만 함)
- **최근 검색어·열람**(계획 1): `KV_DEFAULT.recentQ/recentDocs` 추가(`SYNC_KV_KEYS` 에 없음 확인). `pushRecentQ(raw)`/`pushRecentDoc(id)` 두 함수를 새로 두고 — `openHit()` 이 본문·회의록 적중을 열 때(`Q` 가 있을 때) `pushRecentQ(Q.raw)`, `selectDoc()` 이 불릴 때마다(검색이든 왼쪽 목록 클릭이든) `pushRecentDoc(id)`. 둘 다 중복 제거 후 앞에 놓고 10개로 자름. `#sdrop` 클릭 위임에 `[data-rmq]`(줄별 삭제) · `#sd-clrq`(모두 지우기) 두 가지를 추가(2단계는 자리만 만들어 두고 동작은 없었음).
- **커맨드바 교집합**(계획 2): `#selall`/`#deselall`/`#cats` 세 클릭 핸들러 끝에 `if(S.srch.open){ sdRecompute(); renderSdrop(); }` 추가. 머리글 필터칩(`#sd-fchip`)은 `$("#selall").click()` 만 부르도록 정리(이제 `#selall` 자신이 재계산까지 하므로 중복 호출 제거).
- **좁은 화면**(계획 3, `@media (max-width:860px)`, 1601줄 근처): `#sdrop` 전체 폭·`.sd-row` 세로 쌓기·`.sd-rail` 을 가로 스크롤 그룹 칩 줄로(레일 라벨·회의 종류 막대·기간 세그먼트는 숨김) · `.sd-s` 2줄 클램프 · `#hitbar` 를 `position:fixed;bottom:0;height:44px` 어두운 색으로 · `#searchwrap.focus ~ #topright{display:none}`. `sdDocBlockHtml()` 의 메타 줄은 `isNarrow()` 일 때 연도 대신 회의 종류로 축약(`임원회의 · 08 W3 · 12p`).
- **설명서**(계획 4): TUT 3절 표 3행 문구를 "자료 이름 + PDF 본문 + 회의록 — 그룹별로, 몇 쪽에서 걸렸는지"로, "어디에"·"하는 법" 2단계를 `Ctrl K`/`/`·드롭다운·`↑↓`/`Enter`·문법 도움 `?` 설명으로 바꿈. "검색 문법" 표(8행)를 `SD_GRAMMAR` 배열에서 그대로 만들어 붙임(중복 문구 대신 같은 배열 재사용 — 문법이 바뀌면 설명서도 같이 바뀜).
- **옛 코드 정리**(계획 5): `searchDocs`/`snippetOf`/`openPop`/`closePop`/`renderPop`/`#popcount`/`#searchpop` 등을 실제 코드에서 찾아봤으나 **이미 2단계에서 전부 삭제되어 있었음** — 2단계 완료 기록에 "옛 모달·호환 함수... 2단계에서 삭제"라 적혀 있던 그대로. 이 단계에서는 손댈 게 없어 확인만 하고 지나감. `closePop`(5943줄대)은 그리기 도구용 별개 지역 함수라 그대로 둠(계획에 적힌 대로).
- **성능 확인**(계획 6): 정적 서버 + 가짜 자료(200건 × 60쪽 × 1,500자)로 측정 — 처음엔 **드문 낱말 재검색이 cold 상태에서 220ms** 정도로 100ms 를 넘었음. 원인을 나눠 재 보니 `_pgQ`(쪽별 평문 색인) 생성 비용이 아니라, **`runSearch` 의 span 채우기(`spansFor`)가 걸린 자료마다 `sqzPages(d)` 로 자료 전체 쪽(안 걸린 쪽까지)을 매번 새로 만드는 것**이 진짜 원인이었음(자료 100/200건이 걸리는 현실적인 검색어에서 **약 1.5초**까지 나옴). 계획에 적힌 `_pgQ` 미리 만들기(`prewarmPgQ()`, `loadAll()` 뒤 `setTimeout` 으로 8ms 씩 나눠 돌림 — `requestIdleCallback` 은 이 세션의 헤드리스 환경에서 전혀 안 불려 안 씀)는 그대로 넣었지만, 실제 병목은 따로 고쳐야 했음: `runSearch` 의 span 채우기를 `sqzPages(d)`(자료 전체) 대신 **적중한 쪽만 그 자리에서 `sqz(d.pageTexts[i])`** 하도록 바꿈(캐시·API 는 그대로, `docfind` 가 쓰는 `sqzPages`/`SQLRU` 는 안 건드림). 그 결과 자료 100/200건 걸리는 검색이 **1.5초 → 0.1초** 로, 드문 낱말(20/200건)은 **220ms → 5~9ms** 로 줄었음. 자료 전체(200/200건·189,200곳, 현실적으로 있기 어려운 극단값)는 여전히 2.6초대 — 1단계가 이미 "오타 검색(모든 쪽 적중) 1.0 s" 로 적어 둔 것과 같은 종류의 한계로 남겨 둠(아래 남은 문제).

**검증** — Claude in Chrome 확장은 이번에도 연결되지 않았고, 이 세션의 도구로는 `file://` 를 열 수 없어서(파일 열기 시도 시 오류) 이번 단계는 **정적 서버(Browser pane) + 가짜 자료**로만 검증함:
- 콘솔 오류 0(빈 상태 첫 로드, 가짜 자료 로드, 검색·드롭다운 조작 전 과정에서).
- 검색 → 적중 줄 클릭 → `openHit` → `KV.recentQ`·`KV.recentDocs` 에 저장 확인. Zero-state 를 다시 열어 🕘 최근 검색어·📂 최근 열람 문서 줄이 실제로 뜨는 것 확인. 줄의 ✕(`data-rmq`) 로 하나만 삭제, `모두 지우기`(`#sd-clrq`) 로 전부 삭제되는 것을 클릭 이벤트 디스패치로 확인.
- 커맨드바 칩(`#cats` 낱개 · `Select All` · `Deselect All`)을 드롭다운이 열린 채로 눌러 머리글 집계(`자료 1건 · 적중 3곳` → `0건` → 원복)가 다시 그려지는 것 확인. 머리글 필터칩 ✕ 도 그대로 동작.
- 모바일 뷰포트(375×812)에서 드롭다운이 화면 전체 폭, 레일이 가로 스크롤 그룹 칩 줄(회의 종류·기간 숨김)로, 메타 줄이 "임원회의 · 08 W3 · 2p" 로 축약되는 것을 스크린샷으로 확인. 적중 쪽을 열어 `#hitbar` 가 화면 아래 고정(어두운 색, 44px)으로 뜨는 것, 검색창 포커스 때 `#topright` 가 `display:none` 되는 것(computed style)으로 확인.
- 설명서 3절을 열어 "검색 문법" 8행 표가 `SD_GRAMMAR` 와 같은 내용으로 뜨는 것, `#tutsearch` 에 "문법" 을 치면 3절이 걸리는 것 확인.
- 성능은 위 "실제로 한 것"에 적은 수치대로.
- **사용자 실제 자료로는 아직 안 돌려 봄** — `file://` 로 열어 최근 검색어·최근 열람이 실제로 쌓이는지, 아이패드 좁은 화면에서 `#hitbar` 겹침이 없는지 한 번 확인 권장.

**남은 문제 / 앞으로 참고할 메모**
- **극단적으로 많은 쪽이 한꺼번에 걸리는 검색은 여전히 느림**(자료 전체·189,200곳 기준 2.6초). 실제 회의 자료에서는 한 낱말이 거의 모든 쪽·모든 자료에 있는 일이 드물어 우선순위를 낮췄음(1단계가 오타 검색에 대해 이미 남겨 둔 것과 같은 성격의 한계). 더 고치려면 `spansFor` 를 "걸린 자료 전부"가 아니라 "지금 화면에 보이는 자료만"으로 되돌려야 하는데, 이는 2단계가 "+ 자료 N건 더 보기" 를 재검색 없이 처리하려고 일부러 넓힌 범위라 5단계 범위를 넘는 구조 변경으로 보고 남겨 둠.
- `requestIdleCallback` 은 최소 이 세션의 헤드리스 Browser pane 에서는(탭을 앞으로 가져와도) 전혀 불리지 않았음 — 실제 사용자 브라우저(Chrome·최신 Safari)에서는 지원되지만, 혹시를 대비해 `prewarmPgQ()` 는 처음부터 `setTimeout` 기반으로만 짰음(계획은 `requestIdleCallback` 을 가정했었음).
- 좁은 화면 `#hitbar` 가 화면 하단에 고정되면서 원래 그 자리에 있던 레이아웃 공간은 사라짐(오른쪽 패널의 세로 탭 글자와 일부 겹침을 스크린샷에서 확인) — 4단계가 이미 "5단계 3항에 그대로 남아 있음" 이라 적어 둔 것과 동일한 설계이고, `z-index` 상 막대가 위에 그려져 실사용에 지장은 적어 보이지만 아이패드 실기에서 한 번 더 보면 좋음.
- `sqzPages`/`SQLRU`(LRU 24, `docfind` 전용)는 이번 성능 수정과 무관하게 그대로 둠 — 이름 정리(계획 5의 "sqzPages 캐시 이름 정리")는 지금 이름이 이미 명확해(`SQLRU`/`SQLRU_MAX`) 더 손대지 않음.
...
- **2026-09-18 · 5단계 · `58fa9c9`** — 마무리(마지막 단계). 최근 검색어/열람 저장(`recentQ`/`recentDocs`, 기기별) + Zero-state 줄 삭제·모두 지우기 연결, 커맨드바 칩을 드롭다운 열린 채로 눌러도 즉시 재계산, 좁은 화면(≤860px) 전용 레이아웃(레일→가로 그룹 칩, `#hitbar` 화면 하단 고정, 메타 축약), 설명서 3절 갱신(문법 8행 표를 `SD_GRAMMAR` 에서 그대로 생성). 계획의 "옛 코드 정리" 항목은 2단계에서 이미 끝나 있어 확인만 함. **성능 확인 중 실제 병목을 하나 찾아 고침**: `runSearch` 의 span 채우기가 걸린 자료마다 `sqzPages(d)` 로 자료 전체 쪽을 새로 만들던 것을 적중한 쪽만 `sqz()` 하도록 바꿔, 자료 100/200건이 걸리는 검색이 1.5초 → 0.1초로 줄었음(1단계가 만든 `runSearch` 내부 수정이지만 5단계 성능 확인 중 발견·처리). 자세한 내용·남은 문제는 [5단계] 절.
- **2026-09-18 · 완료 후 버그 수정(단계 없음)** — 두 가지를 고침(둘 다 사용자가 실사용 중 발견).
  1. `findHits`/`hasHit` 추가 — 로마자·숫자로만 된 조각(`in`·`Lock` 등)은 낱말 경계를 지켜서 찾도록 바꿈. 이전엔 부분 문자열로만 견줘서 `in` 이 `Marketing` 안에서, `lock` 이 `unlock`/`block` 안에서도 걸려 AND 가 사실상 느슨해짐(예: `쿠팡+Lock in`). 한글은 PDF 추출 시 낱말 안 공백이 들쭉날쭉해 경계를 못 믿으므로 그대로 부분 문자열로 둠. `termHits`·`findSpans`·`runSearch` 의 `has`·`neg` 판정 모두 이걸 쓰도록 교체.
  2. **AND 의 범위를 자료 전체 → 같은 자리(쪽 하나 / 회의록 / 제목) 단위로 바꿈.** 1-3 절의 원래 설계("1차: 모든 조각이 자료 어딘가에 있으면 통과")는 `아토믹스+쿠팡` 을 쳤을 때 아토믹스는 3p 에만, 쿠팡은 5p 에만 있어도(서로 다른 장표) 그 자료가 걸리는 문제가 있었음 — 사용자가 "한 장표 안에 둘 다 있어야 한다"고 지적. `runSearch` 를 두 단계로 바꿈: 1차(싸게 거르기)는 그대로 자료 전체 기준 후보만 추리고, 2차에서 쪽마다(그리고 회의록·제목도 각각 하나의 '자리'로) `Q.groups.every(...)` 를 다시 적용해 그 자리 안에서 AND 가 다 만족될 때만 적중으로 셈. 어느 자리에도 다 같이 걸린 곳이 없으면 자료째 결과에서 빠짐. `#태그`/`year:` 만 쓰고 조각(`groups`)이 하나도 없는 검색은 원래대로 자료만 걸리고 자리별 적중은 없음(0곳)으로 유지(`hasGroups` 가드). OR(`/`)·제외(`-`)·태그 전용 검색·성능(LRU span 캐시)은 headless 로 합성 자료를 만들어 재검증, 실제 자료로는 아직 검증 못 함 — file:// 로 열어 확인 필요. PLAN-SEARCH.md 1-3 절의 "1차: 자료 걸러내기 — 모든 조각이 … 있으면" 설명은 이제 "후보 추리기"로만 유효하고, 최종 판정은 2차(같은 자리 AND)가 기준.
